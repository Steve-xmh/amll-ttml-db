...
/**
 * HACK: [2025-6-7, apoint123] 用于规范化空格的函数
 * 
 * 此实现虽然功能上有效，但这是一个为了短期稳定而引入的技术债。
 * 直接修改上游的 ttml-parser.js 是最佳解决方案，但是让我来修改，
 * 还不如直接让我重新写一个。parseParseLine 的递归调用和副作用我看了半天仍然一头雾水。
 * 
 * 建议引入 TS 版本的解析器（虽然它们都是一样的），或者重写整个解析逻辑。
 * 
 * 这个逻辑甚至也可以直接移除了，因为它确实没多大用处。
 * 
